...
#define SF_SEND_SIZE 1024*1024


/**
 * 使用SendfileIOPump来下载文件实例，用法
 * 
 * http://192.168.99.60:3080/sf/root/flvs/1.flv?speed=1048576
 * 
...
public:
    ...
    virtual void service(Request& req, Response& resp){
        ...
        if(speed){
            ...
            speedval = SF_SEND_SIZE; //默认速度为SF_SEND_SIZE字节
        }    
        ...
        if(stat(flvfile, &filestat)!=0){
            LOG_INFO("SendfileIOPumpHttpServlet, file not exist! errno:%d", errno);
            resp.setStatus(400, "error");
            return;
        }
        if(S_ISDIR(filestat.st_mode)){
            LOG_INFO("SendfileIOPumpHttpServlet, is dir");
        ...
        ioPump = new SendfileIOPump(httpHandler->getClientFD(), flvfd, speedval, SF_SEND_SIZE);
