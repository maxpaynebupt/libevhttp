...
#include <evhttp/IIOPump.h>
#include <evhttp/RWIOPump.h>
#include <evhttp/HttpServlet.h>
#include <evhttp/events.h>
#include <evhttp/event/IEventListener.h>
#include <evhttp/IIOReader.h>
#include <evhttp/IIOWriter.h>
#include <evhttp/EvNIOReader.h>
#include <evhttp/EvNIOWriter.h>

#define RW_SEND_SIZE 1024*1024


/**
 * 使用RWIOPump来下载文件实例，用法
 * 
 * http://192.168.99.60:3080/rw/root/flvs/1.flv?speed=1048576
 * 
 * 新0.0.3版本已经支持Response.sendfile()功能，推荐使用
 */
...
public:
    ...
    virtual void service(Request& req, Response& resp){
        ...
        if(speed){
            ...
            speedval = RW_SEND_SIZE; //默认速度为1M字节
        }    
        ...
        if(stat(flvfile, &filestat)!=0){
            LOG_INFO("RWIOPumpHttpServlet, file not exist! errno:%d", errno);
            resp.setStatus(400, "error");
            return;
        }
        if(S_ISDIR(filestat.st_mode)){
            LOG_INFO("RWIOPumpHttpServlet, is dir");
        ...
        ioPump = new RWIOPump(ioReader, ioWriter, speedval, RW_SEND_SIZE);
